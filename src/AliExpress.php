...
{    
    ...
    public function getProduct($productId, $fields = null){
        ...
        if($responce && $responce->errorCode == 20010000){
			...
			$properties = $client->getProductProps($productURL);
			$images = $client->getProductImages($productURL);
			$variations = $client->getProductAttr($productURL);
			$colors = array();
			foreach($variations as $var){
				if($var['color'] != '')
				$colors[] = ucfirst($var['color']);
			}

			$shortDescription = $this->aeShortDesc($properties, $colors);

			$responce->result->shortDescription = $shortDescription;
			$responce->result->longDescription = $longDescription;
			$responce->result->properties = $properties;
			$responce->result->images = $images;
			$responce->result->variations = $variations;
        }
        return $responce;
    }
